...
$sql1 = "select * from driver_register";
...
if($resul){
  while ($data = mysql_fetch_array($resul)) {
   ...
   echo "
    <tbody>
      <tr>
        <td><a href='driver_delete.php?rid=$register_id'><button  type='submit' class='btn btn-danger' name='delete' >delete</button></a><a href='driver_accept.php?rid=$register_id & uname=$user_name & name=$name & date=$joined_date & rn=$root_name & rnum=$root_number & pn=$phone  & bn=$bus_number ><button  type='submit' class='btn btn-success' name='ok' >accept</button></a><a class='details'  rid= '$register_id' username='$user_name' name='$name' date='$joined_date' rn='$root_name' rnum='$root_number' pn='$phone' bn='$bus_number'><button  type='submit' class='btn btn-info' name='ok' >view details</button></a></td>
        <td>$register_id</td>
        <td>$root_name</td>
        <td>$root_number</td>
        <td>$bus_number</td>
