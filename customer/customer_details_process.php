...
if($resul){
  while ($data = mysql_fetch_array($resul)) {
   ...
   echo "
    <tbody>
      <tr>
        <td><a href='admin_cus_delete.php?cid=$customer_id'><button  type='submit' class='btn btn-danger' name='delete' >delete</button></a><a class='details' role='button' rid='$register_id' username='$user_name'  name='$name'  date='$joined_date'><button  type='submit' class='btn btn-info' name='ok' >view details</button></a></td>
        <td>$customer_id</td>
        <td>$name</td>
        <td>$user_name</td>
        <td>$joined_date</td>
